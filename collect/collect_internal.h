...
uint64_t g_record_member_lookup_cache_query_revision = 0;
...
void invalidate_record_member_lookup_caches_if_needed() {
    ...
    const CollectQueryContext* active_query_context =
        get_active_collect_query_context();
    uint64_t current_query_revision =
        active_query_context ? active_query_context->revision() : 0;
    if (current_epoch == g_record_member_lookup_cache_epoch &&
        active_ast_ctx_id == g_record_member_lookup_cache_ast_context_id &&
        current_query_revision == g_record_member_lookup_cache_query_revision) {
        return;
    }
    g_record_member_lookup_cache_epoch = current_epoch;
    g_record_member_lookup_cache_ast_context_id = active_ast_ctx_id;
    g_record_member_lookup_cache_query_revision = current_query_revision;
